...
int main(int argc, char** argv) {
    ...
    if (argc < 7) {
        std::cout << "Usage: " << argv[0] << " <io_file> <layer_file> <assembly_process_file> <wafer_process_file> <test_file> <netlist_file> <blocks_file>" << std::endl;
        std::cout << "Example: " << argv[0] << " io_definitions.xml layer_definitions.xml assembly_process_definitions.xml wafer_process_definitions.xml test_definitions.xml block_level_netlist.xml block_definitions.txt" << std::endl;
        return 1;
    }

    //////////////////////////////////////////
    // Initialize the Library C++ Object    //
    //////////////////////////////////////////

    // Library Files from command line arguments
    std::string io_file = argv[1];
    std::string layer_file = argv[2];
    std::string assembly_process_file = argv[3];
    std::string wafer_process_file = argv[4];
    std::string test_file = argv[5];
    std::string netlist_file = argv[6];
    std::string blocks_file = argv[7];

    std::cout << "Using the following files:" << std::endl;
    std::cout << "  IO File: " << io_file << std::endl;
    std::cout << "  Layer File: " << layer_file << std::endl;
    std::cout << "  Assembly Process File: " << assembly_process_file << std::endl;
    std::cout << "  Wafer Process File: " << wafer_process_file << std::endl;
    std::cout << "  Test File: " << test_file << std::endl;
    std::cout << "  Netlist File: " << netlist_file << std::endl;
    std::cout << "  Blocks File: " << blocks_file << std::endl;

    std::cout << "Initializing" << std::endl;
    LibraryDicts* libraryDicts = init(io_file, layer_file, wafer_process_file, assembly_process_file, test_file, netlist_file, blocks_file);
    ...
    if (report_baseline_cost) {
        ...
        std::cout << "Elapsed time for getCostFromScratch: " << elapsed_time << " microseconds" << std::endl;
        std::cout << std::endl;
